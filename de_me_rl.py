...
def find_tube_issues(lvl):
    ...
    lvl_lines = lvl.split("\n")
    tube_issue = 0
    for i in range(len(lvl_lines)-2):
        l = lvl_lines[i]
        test_tube = 0
...
def calc_fitness(latent_arr, no_seg, model, dataset, bin1, bin2, bin3):
    ...
    behaviors.append(b3)
    return fitness, playability, tube_error, recon_loss, behaviors, linearity, lineancy, diff
...
class Chromosome():
    ...
    def eval(self, no_seg, model, dataset, bin1, bin2, bin3):
        self._fitness, self._playability, self._tube_loss, self._recon_loss, self._behaviors, self._linearity, self._lineancy, self._dist = calc_fitness(
    ...
    def load(self, file_name):
        ...
        with open(file_name, 'r') as f:
            ...
            self._recon_loss = temp["recon_loss"]
...
@ray.remote
def apply_mutation(target, donor1, donor2, donor3, _no_segments, _model, _dataset, _scaling_factor,
                   _crossover_rate, _bin1, _bin2, _bin3):
    r1 = donor1._genes
    r2 = donor2._genes
    r3 = donor3._genes
...
@ray.remote
def evaluate(latent, _no_segments, _model, _dataset, _bin1, _bin2, _bin3):
    c = Chromosome(latent, latent)
    c.eval(_no_segments, _model, _dataset, _bin1, _bin2, _bin3)
    return c


class DE_ME:
    def __init__(self, pop_size, no_seg, vae_model, dataset, bin1, bin2, bin3):
    ...
    def run_de_me(self, sf, cr, evals, path, cpus):
        ...
        for e in range(evals):
            while len(pending) < cpus and cnt < evals:
                ...
                pending.append(apply_mutation.remote(t, d1, d2, d2, n_seg, modl, ds, sf, cr, b1, b2, b3))
                cnt += 1
...
def main():
    ...
    save_path = "./de_me_vae3/"
    exp = "./tst"   #"r_vae3_eq_1"
    os.makedirs(save_path + exp)

    details_file = save_path + exp + "/details.json"
    with open(details_file, 'w') as f:
        temp = {
            ...
            "vae_model": "smb_vae_3_1/equal_loss",
            "fitness": fitness,
    ...
    de_me = DE_ME(pop_size, no_seg, vae, dataset, bin1, bin2, bin3)
